...
def apply_permission(permission, permissions_path):
    try:
        ...
        target, permission_type = permission
...
def process_document(file_path):
    try:
        ...
        for username in usernames:
            apply_permission((username, 'write'), permissions_directory)

        for group in groups:
            apply_permission((group, 'write'), permissions_directory)
...
# Create GUI window
root = tk.Tk()
root.title("User Permissions Setup")

# Create file selection button
browse_button = tk.Button(root, text="Select Document", command=browse_file)
